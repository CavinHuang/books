...
我们知道平均值的计算公式是数组的总和 ![\sum_{i=1}^n x_i](https://juejin.cn/equation?tex=%5Csum_%7Bi%3D1%7D%5En%20x_i) 除以数组的长度 ![n](https://juejin.cn/equation?tex=n)，假设我们要给数组 ![x](https://juejin.cn/equation?tex=x) 的子集，前 ![n \- 1](https://juejin.cn/equation?tex=n%20-%201) 个元素的数组进行求平均，就可以得到以下两条公式。

![\begin{gather}
m_n = \frac{\sum_{i=1}^n x_i}{n} \\
m_{n-1}= \frac{\sum_{i=1}^{n-1} x_i}{n-1}
\end{gather}](https://juejin.cn/equation?tex=%5Cbegin%7Bgather%7D%0Am_n%20%3D%20%5Cfrac%7B%5Csum_%7Bi%3D1%7D%5En%20x_i%7D%7Bn%7D%20%5C%5C%0Am_%7Bn-1%7D%3D%20%5Cfrac%7B%5Csum_%7Bi%3D1%7D%5E%7Bn-1%7D%20x_i%7D%7Bn-1%7D%0A%5Cend%7Bgather%7D)

我们通过对 2 式对 1 式进行变形就可以得到我们所需要的公式。

![m_n = \frac{{m_{n-1} \times (n-1) + x_n}}{n}](https://juejin.cn/equation?tex=m_n%20%3D%20%5Cfrac%7B%7Bm_%7Bn-1%7D%20%5Ctimes%20(n-1)%20%2B%20x_n%7D%7D%7Bn%7D)

这条公式表示了长度为 ![n](https://juejin.cn/equation?tex=n) 的数组 ![x](https://juejin.cn/equation?tex=x) 的平均值等于数组 ![x](https://juejin.cn/equation?tex=x) 的前 ![n \- 1](https://juejin.cn/equation?tex=n%20-%201) 个元素的平均值乘以 ![n-1](https://juejin.cn/equation?tex=n-1) 再加上第 ![n](https://juejin.cn/equation?tex=n) 个元素后除以 ![n](https://juejin.cn/equation?tex=n)。其中 ![x_n](https://juejin.cn/equation?tex=x_n) 就可以理解为数据流中的数据块，我们只需要维护上一个平均值和上一个数据集长度即可。以下例子假设数据集不断“吐出”数值 `1,2,3,4` 并以此类推。
