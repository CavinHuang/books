...
const foo = new Foo();
foo.value = 'LINBUDU'; // HijackSetter: LINBUDU
console.log(foo.value); // LIN_BU_DU
```

在这个例子中，我们通过装饰器劫持了 setter ，并通过它来篡改了 getter 的返回值。需要注意的是，我们并不能做到篡改 setter，这里 `descriptor.set` 的赋值中虽然只有 1 个 log 语句，而没有实际的赋值，但原来的 setter 仍然会执行，也就是 `foo.value = 'LINBUDU'` 这么一句。我们无法在 setter 内部去调用原本的 setter ，否则就会造成一个死循环：setter-setter-setter-...。
...
> 关于执行顺序与应用顺序，执行是**装饰器求值得到最终装饰器表达式**的过程，而执行则是**最终装饰器逻辑代码执行**的过程：
> 
> ```typescript
> function deco() {
>   // 执行
