...
假如说需要将这个计算结果展示在图表上，那么根据目前所设定的坐标系可知我们需要添加一条横向的水平线，而这条水平线的纵向位置应该为 ![y](https://juejin.cn/equation?tex=y) 坐标轴上该数值所对应的位置。
...
SPC 控制图所使用的数据主要需要计算数据的平均值和标准差（Standard deviation，并非标准误 Standard error）。平均值的计算我们使用 Lodash 中的 `_.mean` 即可，但 Lodash 并没有提供标准差 ![\sigma](https://juejin.cn/equation?tex=%5Csigma) 的计算方法，所以我们这里也需要自行实现一下标准差的计算方法。![N](https://juejin.cn/equation?tex=N) 为数组 ![x](https://juejin.cn/equation?tex=x) 的长度，![\overline{x}](https://juejin.cn/equation?tex=%5Coverline%7Bx%7D) 为数组 ![x](https://juejin.cn/equation?tex=x) 的平均值。

![\begin{align*}
\overline{x} &= \frac{\sum^{N}_{i=1} x_i}{N} \\
\sigma &= \sqrt{\frac{\sum^{N}_{i=1}(x_i - \overline{x})^2}{N - 1}}
\end{align*}](https://juejin.cn/equation?tex=%5Cbegin%7Balign*%7D%0A%5Coverline%7Bx%7D%20%26%3D%20%5Cfrac%7B%5Csum%5E%7BN%7D_%7Bi%3D1%7D%20x_i%7D%7BN%7D%20%5C%5C%0A%5Csigma%20%26%3D%20%5Csqrt%7B%5Cfrac%7B%5Csum%5E%7BN%7D_%7Bi%3D1%7D(x_i%20-%20%5Coverline%7Bx%7D)%5E2%7D%7BN%20-%201%7D%7D%0A%5Cend%7Balign*%7D)
...
LCL = \overline{x} - 3 \times \sigma](https://juejin.cn/equation?tex=UCL%20%3D%20%5Coverline%7Bx%7D%20%2B%203%20%5Ctimes%20%5Csigma%20%5C%5C%0ALCL%20%3D%20%5Coverline%7Bx%7D%20-%203%20%5Ctimes%20%5Csigma)
