...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/8/27/1657752198db3025~tplv-t2oaga2asx-image.image)
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/8/27/1657752595e923a4~tplv-t2oaga2asx-image.image)
...
3.  通过 <https://api.weixin.qq.com/sns/jscode2session> 换取 openid 和 session\_key。
...
| nick\_name | varchar\(255\) | 用户的昵称 |
| avatar\_url | varchar\(255\) | 用户头像 |
| gender | integer | 用户的性别 |
| open\_id | varchar\(255\) | 用户 open\_id |
| session\_key | varchar\(255\) | 用户 session\_key |
...
利用微信开放接口 <https://api.weixin.qq.com/sns/jscode2session> 获取 openid 与 session\_key。在 Node.js 服务端使用 axios 插件发送 HTTP 请求。并需要自行申请小程序的 AppID 与 AppSecret。可以用小程序账号登录微信公众平台，在设置 \-> 开发设置 \-> 开发者 ID 中可以找到 AppID 和 AppSecret。
