...
这次我们利用执行 `sql` 语句的形式，从数据库中获取需要的数据，`app.mysql.query` 方法负责执行你的 `sql` 语句，上述 `sql` 语句，解释成中文就是，“从 bill 表中查询 user\_id 等于当前用户 id 的账单数据，并且返回的属性是 id, pay\_type, amount, date, type\_id, type\_name, remark”。
